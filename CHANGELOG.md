...
- **Added** a **Venus D** (`VNSD-0`) battery model to the bundled simulator: set a battery's `meter_dev_type` to `VNSD-0` and it steers self-consumption with the Venus D's integer proportional-integrator loop — a configurable `ctrl_ratio` gain, charge/discharge clamps and a ±11 W (single-phase) / ±15 W (combined) deadband — instead of the Venus-class float ramp.
